...
[R,'When approaching an uncontrolled intersection, what should you do?',['Slow down, look for other traffic and proceed only when it is safe','Accelerate so that you cross first','Stop in the middle of the intersection','Use your horn and continue without looking'],0,'An uncontrolled intersection must be approached cautiously, with careful observation before proceeding.'],
...
[R,'When a pedestrian is crossing at a marked pedestrian crossing, you must:', ['Stop and give way to the pedestrian','Sound the horn and continue','Drive around the pedestrian','Accelerate through before the pedestrian reaches your lane'],0,'A driver must give way to a pedestrian using a marked pedestrian crossing.'],
...
[R,'While driving, may you hold a cellphone in your hand or against your body?',['No','Yes, when traffic is slow','Yes, below 40 km/h','Yes, on a freeway'],0,'A driver may not hold a cellphone or other communication device in a hand or against the body while the vehicle is being driven.'],
...
[R,'What must you do at a stop sign?',['Come to a complete stop at the stop line, then proceed only when safe','Slow down and continue if no vehicle is visible','Stop only when another vehicle is approaching','Sound the horn and drive through'],0,'A stop sign requires a complete stop at the stop line, or at the appropriate stopping position if no line is present.'],
...
[S,'What should you do when a solid white line prohibits crossing?',['Remain on your side of the line','Cross it whenever traffic is light','Park on the line','Use the shoulder to pass'],0,'A prohibitory solid line must not be crossed except where a lawful exception applies.'],
...
[S,'What does a yellow line along the side of the road mark?',['The edge of the roadway','The centre of the road','A pedestrian crossing','A stop line'],0,'A yellow edge line marks the edge of the roadway and separates it from the shoulder.'],
  ...
  return shuffle(
    QUESTION_BANK.map((sourceQ, id) => {
      ...
      const fallbackVisualMap = {
        ...
        "What should you do when a solid white line prohibits crossing?": ["fallback_solid_line", "road_marking"],
        "A broken lane line normally means:": ["fallback_broken_line", "road_marking"],
        "What does a yellow line along the side of the road mark?": ["fallback_yellow_edge", "road_marking"],
